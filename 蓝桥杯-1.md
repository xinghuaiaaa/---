...
欧拉降幂用于计算形如 `a^b mod m` 的问题。其核心思想是通过**欧拉定理**来简化指数的计算。欧拉定理告诉我们，当 `a` 和 `m` 互质时，可以通过欧拉函数 φ(m)来减少计算的大数幂。
...
重点:: 费马小定理是欧拉定理的一个特例。对于质数 p，φ(p) = p - 1，因此费马小定理可以看作是欧拉定理在 n 为质数时的特殊情况。
